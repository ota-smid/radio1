...
def my_event_callback_function(pin):
  global station_num
  global vysilame_offline
  global vysilame_online
  print pin
  print vysilame_offline
  print vysilame_online

  # pin 11 - stisknuto tlacitko pro offline vysilani 
  if (pin == 11):
    print "debug 0"
    if (vysilame_offline == 0):    # nevysilame offline a chceme zapnout
#      if (vysilame_online == 1):   # vysilame online
...
#      return
    ...
    if (vysilame_offline == 1):   # vysilame offline a chceme vypnout
      vysilame_off = 0
      GPIO.output(16, True)
      GPIO.output(18, True)
      GPIO.output(22, True)
      ...
      print "debug 3"
      print vysilame_off
      retvalue = os.system("sudo /home/pi/pifm/pifmplay next &")
#      return
      
  # pin 13 - stisknuto tlacitko pro nastaveni stanice
  if (pin == 13):
    ...
    if station_num == 1:
      station_num = 2
      print ('Prepocitavam! Preskakuju jednicku', station_num)
  ...
  if (pin == 15):
    if (vysilame_online == 0): # online vysilani vypnut a chceme zapnout
      if (vysilame_offline == 1):
        station_num = 2
        GPIO.output(16, True)
        GPIO.output(18, False)
        GPIO.output(22, True)        
      ...
      if (vysilame_online == 1): # online vysilani , zastavit
        vysilame_online = 0
        print ('Koncime!')
